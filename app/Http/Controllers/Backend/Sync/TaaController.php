...
use App\Services\Sync\TaaServices;
use Illuminate\Http\Request;
use App\Models\History;

class TaaController extends Controller
{
  public function __construct(TaaServices $service)
  {
  ...
  public function index()
  {
    ...
    if($role_id!=1)
    {
      $sales = UserSales::where('company_id',2)
    ...
    return view('backend.sync.taa.index')->with($data);
  }
  ...
  public function sync(Request $request)
  {
    ...
    $url = 'http://sfa.laut-timur.com/sfaerp3/api/order_detail_sales';
  ...
  public function detail(Request $request)
  {
    ...
    return view('backend.sync.taa.detail')->with($data);
  }

  public function push(Request $request)
  {
    $id = $request->id;
    $username = auth()->user()->username_sap_taa;
    $password = auth()->user()->password_sap_taa;
