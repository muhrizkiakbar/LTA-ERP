...
class TaaServices
{
  ...
  public function getDataFromSfa($url, $post)
  {
    $data_sfa = callApiWithPost($url, $post);
    ...
    if(isset($data_sfa['data']))
    {
      foreach($data_sfa['data'] as $data_header)
      {
        ...
        if($cek_header==0)
        {
          $customer = $this->getCustomerDetailSfa(2, $data_header['CardCode']);
  ...
  public function generateDataLinesSfa($numAtCard, $cardCode, $lines, $whs, $class)
  {
    ...
    {
      $UomData = $this->getUomDetail(2, $lines['ItemCode'], $cardCode, $whs);
      ...
      if (!empty($UomData)) 
      {
        ...
        $available = $this->getAvailable(2,$lines['ItemCode'],$whs);
        ...
        $getUomEntry = getUomEntry($post3,2);
        $UomEntry = isset($getUomEntry) ? $getUomEntry : '' ;

        $getUomEntry2 = getUomEntry($post4,2);
        $UomEntry2 = isset($getUomEntry2) ? $getUomEntry2 : '';
        $total_line = $qty * $UnitPrice;

        $this->syncItemToErp(2,$lines['ItemCode'],$lines['ItemName']);
      }
      else
      {
        $UnitMsr = '';
  ...
  public function jsonData($id)
  {
    ...
    $json = [
      ...
      'NumAtCard' => $header->NumAtCard.'_ERP2.0',
      'Comments' => $header->Comments.'_ERP2.0_'.time(),
  ...
  public function jsonDataLines($id)
  {
    ...
    foreach($json as $lines)
    {
      ...
      $available = $this->getAvailable(2,$itemCode,$whsCode);
      $stok = isset($available) ? $available : 0;
  ...
  public function getUomDetail($company, $itemCode, $cardCode, $whsCode)
  {
    ...
    $urlUom = 'http://36.93.82.10/erp-api-lta/api/getUomDetail';
    $UomData = callApiWithPost($urlUom, json_encode($post_uom));
  ...
  public function getAvailable($company, $itemCode, $whsCode)
  {
    ...
    $url = 'http://36.93.82.10/erp-api-lta/api/cekAvailable';
    $stok = callApiWithPost($url,json_encode($post));
  ...
  public function syncItemToErp($company, $itemCode, $itemName)
  {
    ...
    $url = 'http://36.93.82.10/erp-api-lta/api/getItemDetailSfa';
    ...
    if(isset($get))
    {
      ...
      $cek_item = Item::where('code',$itemCode)->where('company_id',2)->get();
  ...
  public function getCustomerDetailSfa($company, $cardCode)
  {
    ...
    $url = 'http://36.93.82.10/erp-api-lta/api/getCustomerDetailSfa';
    $get = callApiWithPost($url,json_encode($post));
