    ...
    <div class="col-md-12">
      <form method="POST" id="sync">
        <div class="form-group row">
          ...
          <div class="col-md-2">
            {!! Form::select('sales',$sales,null,['class'=>'form-control form-control-sm','id'=>'sales','placeholder'=>'-- Pilih Cabang --','required'=>true]) !!}
          </div>
          @else
          <label class="col-md-1 col-form-label">Pilih Sales</label>
					<div class="col-md-2">
            {!! Form::text('date',null,['class'=>'form-control form-control-sm datepick-min','id'=>'date','data-language'=>'en','data-date-format'=>'yyyy-mm-dd','required'=>true]) !!}
          </div>
          <div class="col-md-3">
  ...
  $(document).ready(function() {
    ...
    var oTable = $('.datatable-basic2').DataTable({
        ...
        $(".detail").click(function(e) {
          var id = $(this).data('id');
          var url = '{{ route('backend.sync.png_detail') }}';
    ...
    $("#sync").on("submit",function(e){
      ...
      var sales = $("#sales").val();
      var token = '{{ csrf_token() }}';
      var url = '{{ route('backend.sync.png_sync') }}';
      $('#overlay').fadeIn();
      $.ajax({
        url : url,
        ...
        success:function(response){
          if (response.message=="sukses") {
            ...
            swalInit.fire({
              ...
              window.location.href = "{!! route('backend.sync.png') !!}";
            });
          } else {
            $('#overlay').hide();
            swalInit.fire({
