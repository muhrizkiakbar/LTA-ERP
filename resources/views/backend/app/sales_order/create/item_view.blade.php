        ...
        <tbody>
          @if (count($row) > 0)
            ...
            <tr>
              ...
              <td class="text-center">{{ round($item['stok'],0) }}</td>
            </tr>
            @endforeach
          @else
          <tr>
  ...
  $(document).ready(function() {
    ...
    var oTable = $('.datatable-basic2').DataTable({
        ...
        $(".check_item").click(function(e) {
          ...
          var csrf = '{{ csrf_token() }}';
          var url = '{{ route('backend.app.sales.selectItemCreate') }}';
          $.ajax({
            url: url,
            type: "POST",
            data : { _token: csrf,id:id,company:company,whsCode:whsCode,cardCode:cardCode,U_CLASS:U_CLASS },
